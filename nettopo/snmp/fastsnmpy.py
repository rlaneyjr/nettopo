...
class SnmpSession:
    ...
    def _run_queries(self, mode, processes):
        ...
        out_q = Queue()
        print(f"Starting worker pool with {int(processes)} processes")
...
def worker_snmpbulkwalk(entity):
    ...
    print(f"Getting {oid} from {target} ")
    mysession = netsnmp.Session(Version=entity['version'], DestHost=target,
                                Community=entity['community'], UseNumeric=1)
    results = []
